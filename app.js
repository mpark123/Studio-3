...
const updatePage = async () => {
  ...
  const result = fruitsArray.filter(fruit => fruit.nutritions.calories <= 50);
  const final = result.map(fruit => {
    return fruit.name + " has " + fruit.nutritions.calories + " calories";
  })

  // TODO: Create a new HTML element to display your data
  for (let i = 0; i < final.length; i++) {
    const newPic = document.createElement('div');
    newPic.className = "child";
    newPic.innerHTML = final[i];

  // TODO: Append your new element to the page
    const existingPic = document.getElementById('cs1300-gallery');
    existingPic.append(newPic);
  }
}
